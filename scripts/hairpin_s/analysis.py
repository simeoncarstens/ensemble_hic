...
em = 'gaussian'
em = 'poisson'
sim_name = 'lambdatempering_wzeros_{}_{}'.format(em, variables)
sim_name = 'lambdatempering_{}_{}'.format(em, variables)
# sim_name = 'lambdatempering_lowkve_{}_{}'.format(em, variables)
# sim_name = 'lambdatempering_sphere_wzeros_{}_{}'.format(em, variables)
sim_name = 'lambdatempering_sphere_{}_{}'.format(em, variables)
...
# sim_path = '/scratch/scarste/ensemble_hic/murks_{}_{}_{}structures_{}replicas/'.format(em, variables, n_structures, n_replicas)
schedule = numpy.load(sim_path + 'schedule.pickle')['beta']

data_file = 'spiral_hairpin_data_{}beads_littlenoise.txt'.format(n_beads)
# data_file = 'optimal_data.txt'.format(n_beads)
...
if True:
    ...
    if True:
        samples = load_sr_samples(sim_path + 'samples/', n_replicas, 12201, 100,0)
...
if em == 'poisson':
    ...
    p = make_lognormal_posterior(n_structures,
                                 data_file,
                                 n_beads=n_beads, smooth_steepness=5.0, beta=1.0,
                                 disregard_lowest=0.0, lammda=1.0,
                                 contact_distance=2.0,
...
if True:
    ...
    vardict = dict(structures=X, k2=0.3)
    if variables == 'sw':
...
if True:
    ...
    ens = numpy.array([x.variables['structures'].reshape(n_structures, n_beads, 3)
                       for x in samples[-1,-1000::1]]).reshape(-1,n_beads,3)
    ens -= ens.mean(1)[:,None,:]
    
    writeGNMtraj(ens, '/tmp/out_{}.pdb'.format(variables))
