...
if rank == 0:
    ...
    print "RANK:", rank
    posterior['lammda'].set(schedule['lammda'][rank - 1])

    # initial_state = np.load(os.path.expanduser('~/projects/ensemble_hic/scripts/nora2012/lambdaonlystates.pickle'))[rank - 1]
    initial_state = np.load(settings['initial_state']['mystates'])[rank-1]
    if size == 100:
	n_structures = int(settings['general']['n_structures'])
	if n_structures > 20:
            initial_state.update_variables(structures=np.tile(initial_state.variables['structures'], n_structures / 20),
					   norm=initial_state.variables['norm'] / float((n_structures / 20)))
	else:
	    pass
